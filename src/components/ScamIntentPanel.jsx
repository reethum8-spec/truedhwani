...
import { AlertCircle, ShieldAlert, Sparkles, Target } from 'lucide-react';
...
export default function ScamIntentPanel({
  ...
  // Ensure all 10 standard categories exist with fallback values
  ...
  const entries = defaultCategories.map((name) => {
    // Check possible matching keys in dict
    let prob = 0.0;
  ...
  return (
    <div className="tactical-card rounded p-4 font-mono select-none">
      ...
      <div className="mt-3 flex items-center justify-between p-3 rounded tactical-well">
        <div>
          <div className="text-[10px] text-slate-400">MALICIOUS INTENT CONFIDENCE</div>
        ...
        <div className="text-right">
          <span className="text-[10px] text-slate-500">COERCION LEVEL</span>
          <div
